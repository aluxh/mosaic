...
//     npx tsx src/cli/exportLocal.ts <renderUrl> [eventId] [outDir] [slow] [fps]
//
...
async function main(): Promise<void> {
  ...
  const [renderUrl, eventId = 'export', outDir = process.cwd(), slowArg = '1', fpsArg = '30'] = args;
  if (!renderUrl) {
    console.error('usage: tsx src/cli/exportLocal.ts <renderUrl> [eventId] [outDir] [slow] [fps]');
    process.exit(1);
  }
  ...
  console.error(
    `Exporting ${renderUrl}/?render=1&slow=${slow} → ${path.join(outDir, 'exports')} (slow=${slow} fps=${fps})\n` +
    `  capture will take ≈${slow}× the show length; set slow≥2.5 for smooth 60fps`,
  );
  const { outputUrl } = await renderVideoHiFi(
    { renderUrl, outDir, eventId, slow, fps },
